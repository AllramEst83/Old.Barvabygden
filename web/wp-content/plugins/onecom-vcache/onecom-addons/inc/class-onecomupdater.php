...
final class ONECOMUPDATER {
	...
	**/
	private $onecom_installed_plugins;
	private $onecom_installed_themes = array();

	public function __construct() {
		/**
		* Stores One.com themes and plugins based on author name and must match to One.com or one.com
		**/
		add_action( 'wp_loaded', array( $this, 'set_onecom_themes_plugins' ) );
	...
	public function one_check_if_oc_plugin( $slug, $onecom_plugins_keys ) {
		if ( in_array( $slug, $onecom_plugins_keys, true ) ) {
	...
	**/
	public function one_plugins_api_callback( $false_oc, $action, $args ) {

		if ( 'plugin_information' === $action && ( ( isset( $_REQUEST['section'] ) && 'changelog' === $_REQUEST['section'] ) || ( isset( $_REQUEST['tab'] ) && 'plugin-information' === $_REQUEST['tab'] ) ) ) {
	...
	**/
	public function onecom_plugin_row_meta( $links, $file ) {
		...
		$new_links = array(
			'view-details' => sprintf( '<a class="thickbox open-plugin-details-modal" href="%s?tab=plugin-information&plugin=%s&section=changelog&TB_iframe=true&width=600&height=800" target="_blank">%s</a>', $url, $plugin, __( 'View details' ) ),
		);
	...
	**/
	private function set_onecom_plugins() {
		...
		foreach ( $plugins as $file => $plugin ) :
			//skip one.com Migrator plugin for generate View details
			if ( 'one.com Migrator' === $plugin['Name'] ) {
				continue;
			}
